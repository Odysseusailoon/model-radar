...
从 X (Twitter) 持续收集 AI 产品的**可引用社会证据**,LLM 自动分类成三类核心证据
(真实 demo / 客户 case / 大佬评价),沉淀成一张结构化、可筛选、可导出的表,供市场
和销售团队直接取用。

**多产品复用是一等公民**:所有产品相关内容(名称、关键词、官方账号、种子 KOL、launch
日期)都是数据库里的配置行。加一个新产品 = 在 `/admin/products` 填一张表单,**零代码改动**。
...
app/
  ...
  collector.py   采集编排:建 query、增量、轮询种子 KOL、护栏
...
| `ALERT_MIN_CONFIDENCE` | | `0.75` | high_signal 告警的最低置信度 |
...
- **采集**(APScheduler,每 `COLLECT_INTERVAL_MINUTES` 分钟):对每个 active 产品,
  用 OR 拼接关键词做 advanced search(`-filter:retweets` 排除转推),按 snowflake tweet_id
  水位线增量;并轮询每个种子 KOL 的最新推文。
- **分类**(Claude Haiku):每条新推文输出严格 JSON(见 `classifier.py` 的完整 prompt)。
  JSON 解析失败重试一次,再失败则以 `classification_failed=True` 入库,**不丢数据**。
- **存储**(Postgres):`evidence` 表 `tweet_id` 唯一索引去重;`category/sentiment/confidence`
  冗余列便于筛选;`review_status` 默认 `pending`。
- **告警**(飞书):`demo/customer_case/expert_review` 且 `confidence≥0.75` 且粉丝≥1万,
  或任意粉丝≥10万的相关提及 → 推 interactive 卡片,经 `alerts_sent` 去重。
- **Dashboard**:`/` 证据流(顶部筛选 + 每卡 Approve/Reject);`/export` 按当前筛选导出 CSV;
  `/admin/products` 产品配置表单。全站 HTTP Basic Auth。
...
6. **全局唯一 `tweet_id`**:同一条推文只归属首个入库的产品;跨产品重复极少且丢弃无害。
